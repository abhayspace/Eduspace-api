"""Store and serve announcement file attachments (any file type)."""
...
STORAGE_DIR = ROOT_DIR / "storage" / "announcement_attachments"
...
async def save_announcement_attachment(school_id: str, file: UploadFile) -> dict:
    ...
    return {
        "attachment_url": f"/api/announcements/attachments/{stored_name}",
        "attachment_name": original,
        "content_type": mime,
    }


def delete_announcement_attachment(school_id: str, filename: str) -> None:
    path, _ = resolve_announcement_attachment(school_id, filename)
...
def resolve_announcement_attachment(school_id: str, filename: str) -> tuple[Path, str]:
